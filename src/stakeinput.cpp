...
// The zPXC block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zPXC is a hash of the serial
...
{
    //Create an output returning the zPXC that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZPXCOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zPXC output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zPXC", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZPXCOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zPXC output", __func__);
...
{
    CzPXCTracker* zpxcTracker = pwallet->zpxcTracker.get();
    CMintMeta meta;
    if (!zpxcTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zpxcTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a PXC stake is the outpoint
