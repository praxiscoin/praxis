...
CzPXCWallet::CzPXCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zpxc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPXCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPXCSeed_deprecated()) {
                    LogPrintf("%s: Updated zPXC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpxc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpxc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPXCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPXCCount(nCountLastUsed);
    else if (!walletdb.ReadZPXCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPXCWallet::Lock()
{
    seedMaster = 0;
}

void CzPXCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPXCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPXC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPXCWallet::LoadMintPoolFromDB()
{
...
void CzPXCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPXCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPXCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpxcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPXCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPXC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpxcTracker which also adds to database
    pwalletMain->zpxcTracker->Add(dMint, true);
...
void CzPXCWallet::SeedToZPXC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPXCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPXCWallet::GenerateDeterministicZPXC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPXCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPXCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
