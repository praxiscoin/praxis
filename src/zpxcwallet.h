...
#ifndef Praxis_ZPXCWALLET_H
#define Praxis_ZPXCWALLET_H
...
public:
    CzPXCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZPXC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZPXC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Praxis_ZPXCWALLET_H
