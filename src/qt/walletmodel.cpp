...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered praxis address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 PXC.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal praxis:URI (praxis:XyZ...?message=example)
            {
...
static void NotifyzPXCReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzPXCReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzPXCReset.connect(boost::bind(NotifyzPXCReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzPXCReset.disconnect(boost::bind(NotifyzPXCReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zpxcTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
