...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::PXC);
    ...
    if (!settings.contains("nAnonymizePraxisAmount"))
        settings.setValue("nAnonymizePraxisAmount", 1000);

    nAnonymizePraxisAmount = settings.value("nAnonymizePraxisAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizePraxisAmount"))
        SoftSetArg("-anonymizepraxisamount", settings.value("nAnonymizePraxisAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in praxis.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizePraxisAmount:
            return QVariant(nAnonymizePraxisAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizePraxisAmount:
            nAnonymizePraxisAmount = value.toInt();
            settings.setValue("nAnonymizePraxisAmount", nAnonymizePraxisAmount);
            emit anonymizePraxisAmountChanged(nAnonymizePraxisAmount);
            break;
        case CoinControlFeatures:
