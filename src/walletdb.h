...
public:
    ...
    bool WriteZPXCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZPXCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZPXCSeed_deprecated(uint256& seed);
    bool EraseZPXCSeed();
    bool EraseZPXCSeed_deprecated();

    bool WriteZPXCCount(const uint32_t& nCount);
    bool ReadZPXCCount(uint32_t& nCount);
